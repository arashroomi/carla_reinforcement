...
class CreateWalkers:
        ...
        while ((i < self.number_of_walkers)):
            try:
                ...
                if loc!=None:
                    ...
                    occlusion_points = np.array([180, 53, 0.4])
                    ped_loc_relative = np.dot(R, pedestrian_xyz-vehicle_xyz)
                    #ped_loc_relative = np.dot(R, pedestrian_xyz-occlusion_points)
                    ...
                    if ((4<x_to_car < 90) &(3<y_to_car<15)):
                    #if ((-10<x_to_car < 30) &(3<y_to_car<15)):
                        ...
                        if (walker == None):
                            ...
                            self.wait_time.append(random.randint(10, 50))
    ...
    def ApplyControl(self):
        ...
        for i in range(len(self.walkers)):
            control.speed = random.uniform(15, 14)/3.6
    ...
    def walker_collision_creator(self,vehicle):
        ...
        for walker in self.walkers:
            ...
            if not self.finished_control[0,i]:
                ...
                self.xtc = -0.5 * 5.8 * (ttc ** 2) + self.vehicle_max_speed * ttc + (
                        ttc * 30)

                if not self.applied_control[0,i]:
                    #start_walking_distance = self.start_walking_distances[i]
                    #if x_to_car<start_walking_distance+random.uniform(3, 5) and x_to_car>1:
                    if x_to_car < self.xtc/4 and x_to_car > 1:
                        control = carla.WalkerControl()
                        control.direction.y = self.ped_velocity_relative[1]
                        control.direction.x = self.ped_velocity_relative[0]
                        control.direction.z = 0
                        control.speed = random.uniform(8, 10)/3.6
